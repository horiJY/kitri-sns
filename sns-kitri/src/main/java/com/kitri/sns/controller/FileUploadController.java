...
//	4. 파일 업로드, 
	...
	public String upload(MultipartHttpServletRequest multiPart, String content, Map<String, String> map, HttpSession session,RedirectAttributes rdat) {
...
//			String uploadPath = req.getRealPath("/image/members/")+ map.get("loginid") + "/" + feedid;
			String uploadPath = "C:/Users/kitri/Desktop/kitri-sns/sns-kitri/src/main/webapp/image/members/"+ map.get("loginid") + "/" + feedid;
...
//		폼 형식의 문서를 작성 후 post로 보내 처리한 뒤 redirect를 하게되면 get방식으로 인해 request의 파라미터가 url뒤에 모두 붙게 된다 이를 처리해주기 위해 addFlashAttribute를 사용한다.
