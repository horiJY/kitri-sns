					...
					function() {
						...
						var likebtnupdate = function(event) {
							...
							$.ajax({
								...
								type : "post",
								data : {
									"feedid" : selectfeed.children('#feedid')
											.val()
								},
								dataType : "json",
								success : function(data) {
									selectfeed.children('#likes').html(
											'<img class="likebtn" src="${path}/image/icon/likecheck.png"/> 좋아요 '
													+ data + ' 개');
								},
															...
															if (data.length > 0) {
																...
																for (let i = 0; i < data.length; i++) {
																	morefeedlist += "<div class='feedwrapper'>"
																			...
																			+ "</div>"
																			+ "<div id='imgListWrapper'>";
																	for (let j = 0; j < data[i].imageURL
																			.split(',').length; j++) {
																		morefeedlist += "<img class='feedimgbox' src='${path}/image/members/"
																				+ data[i].memberId
																				+ "/"
																				+ data[i].feedId
																				+ "/"
																	...
																	morefeedlist += "</div>"
																			+ "    <div id='likes'> <img class='likebtn' src='${path}/image/icon/noti.png' />좋아요 "
																			+ data[i].likes
																			+ "개</div>"
																			+ "    <div id='feedcontent'>"
																			+ data[i].content
